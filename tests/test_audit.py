...
def test_audit_detects_stale_branches(tmp_git_repo_with_stale):
    ...
    data = json.loads(result.stdout)
    stale = [f for f in data["findings"] if f["type"] == "stale_branches"]
    assert len(stale) == 1
...
def test_audit_detects_missing_commit_graph(tmp_git_repo):
    result = _run("audit", "--json", cwd=tmp_git_repo)
    data = json.loads(result.stdout)
    assert data["summary"]["commit_graph"] is False
    missing = [f for f in data["findings"] if f["type"] == "missing_commit_graph"]
...
def test_audit_detects_large_blobs(tmp_git_repo_with_large_blob):
    ...
    data = json.loads(result.stdout)
    large = [f for f in data["findings"] if f["type"] == "large_blobs"]
    assert len(large) == 1
...
def test_audit_detects_fsmonitor_state(tmp_git_repo):
    result = _run("audit", "--json", cwd=tmp_git_repo)
    data = json.loads(result.stdout)
    # On macOS, fsmonitor_disabled finding should appear if not configured
    if platform.system() == "Darwin":
        fsmonitor = [f for f in data["findings"] if f["type"] == "fsmonitor_disabled"]
...
def test_audit_clean_repo_no_medium_findings(tmp_git_repo_with_commit_graph):
    ...
    data = json.loads(result.stdout)
    medium_plus = [f for f in data["findings"] if f["severity"] in ("critical", "high", "medium")]
...
def test_audit_warns_mixed_staged_unstaged(tmp_git_repo):
    ...
    result = _run("audit", "--json", cwd=tmp_git_repo)
    data = json.loads(result.stdout)
    mixed = [f for f in data["findings"] if f["type"] == "mixed_staging"]
    assert len(mixed) == 1
...
def test_audit_quick_skips_large_blobs(tmp_git_repo_with_large_blob):
    ...
    assert data["summary"]["large_blobs"] == 0
