...
- Commits: breaking changes MUST include a `BREAKING CHANGE: <description>`
  footer (or `!` after the type, e.g. `feat!: ...`). Without one of these markers,
  `commitizen` will pick a patch bump even when the body mentions breaks, and the
  CHANGELOG entry generated by the auto-release workflow will not surface the
  break in `### Breaking Changes`. With `major_version_zero = true`, a real
  breaking change should rotate the minor (e.g. `0.22.x → 0.23.0`).
- Commits: NEVER write the literal string `BREAKING CHANGE:` in prose inside a
  commit body unless that commit really introduces a breaking change.
  `commitizen` scans the full message for the marker and will fire a minor bump
  on any commit that mentions it — including `docs:` commits that merely
  document the convention. If you need to refer to the marker descriptively,
  wrap it in backticks and break the colon (e.g. `BREAKING CHANGE:` or
  `the "BREAKING CHANGE" footer`) so the matcher does not trigger. See the
  `v0.23.0` CHANGELOG entry for an example of what happens when this rule
  is violated.
