...
def _render_pr_view(payload: dict[str, Any]) -> None:
    ...
    assignee_names = ", ".join(_actor_name(item) for item in assignees if isinstance(item, dict))
    review_request_names = ", ".join(
        _actor_name(item.get("requestedReviewer"))
        for item in review_requests
        if isinstance(item, dict) and item.get("requestedReviewer")
    )
...
def _render_pr_comments(payload: dict[str, Any]) -> int:
    number = str(payload.get("number", "-"))
    title = str(payload.get("title") or "")
    url = str(payload.get("url") or "")
...
def run_pr(
    *,
    ...
    if action == "checks":
        fields = "name,state,startedAt,completedAt,link,workflow,event"
        rc, out, err = _gh(["pr", "checks", *selected, "--json", fields], cwd=root)
        if rc != 0:
            error(err)
            return 1
        ok_json, payload = _json_or_error(out)
    ...
    if action == "view":
        ...
        rc, out, err = _gh(["pr", "view", *selected, "--json", fields], cwd=root)
        if rc != 0:
            error(err)
            return 1
        ok_json, payload = _json_or_error(out)
    ...
    if action == "comments":
        fields = "number,title,url,comments"
        rc, out, err = _gh(["pr", "view", *selected, "--json", fields], cwd=root)
        if rc != 0:
            error(err)
            return 1
